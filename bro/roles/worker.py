...
async def gpt_tool_call(
    ...
    context_prompt = (
        f"Mission: {mission}\n"
        f"Current Step (execute this specific action): {user_prompt}\n"
        f"History so far (in Markdown):\n{history}\n"
        f"IMPORTANT: Focus ONLY on executing the current step '{user_prompt}'. "
        f"Do NOT skip ahead to later steps. Execute the current step exactly as specified. "
    ...
    if success:
        ...
        print(f"✅ Step completed successfully: {user_prompt}")
    else:
        append_history(run_id, user_prompt, f"{tool} {arguments}", f"FAILURE: {result}")
        print(f"❌ Step failed: {user_prompt}")
    return success
...
async def run_gpt_tool_chain(
    ...
    # clear history file
    with open(get_history_file(run_id=run_id), "w") as f:
        pass
