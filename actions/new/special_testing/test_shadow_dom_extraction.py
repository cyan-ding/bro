	...
	async with async_playwright() as p:
		...
		try:
			...
			for host in hosts_result:
				print(f"  - {host['tagName']} (class: {host['className']}, id: {host['id']})")
		except Exception as e:
		...
		try:
			detailed_result = await page.evaluate("() => window.findShadowHostsDetailed()")
			print(f"findShadowHostsDetailed found {len(detailed_result)} shadow hosts:")
			for shadow in detailed_result:
				print(f"  - {shadow['tagName']} (id: {shadow['id']}, mode: {shadow['shadowRoot']['mode']})")
				if shadow['shadowRoot']['innerHTML']:
					print(f"    Content length: {len(shadow['shadowRoot']['innerHTML'])}")
					print(f"    Text content: {shadow['shadowRoot']['textContent'][:100]}...")
				else:
		...
		try:
			summary_result = await page.evaluate("() => window.extractShadowDOMBestSummary()")
			...
			if summary_result['summary']:
				...
				for shadow in summary_result['summary']:
					print(f"  - {shadow['hostTag']} (class: {shadow['hostClass']}, id: {shadow['hostId']}, mode: {shadow['shadowMode']}, children: {shadow['shadowChildrenCount']})")
			else:
