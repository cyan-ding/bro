	...
	async with async_playwright() as p:
		...
		try:
			...
			for iframe in iframes_result:
				print(f"  - {iframe['tagName']} (class: {iframe['className']}, id: {iframe['id']}, src: {iframe['src']})")
		except Exception as e:
		...
		try:
			...
			for iframe in detailed_result:
				print(f"  - {iframe['tagName']} (id: {iframe['id']}, src: {iframe['src']})")
				if iframe['iframeContent']:
					if iframe['iframeContent'].get('error'):
						print(f"    Error accessing content: {iframe['iframeContent']['error']}")
					else:
						print(f"    Title: {iframe['iframeContent'].get('title', 'N/A')}")
						print(f"    URL: {iframe['iframeContent'].get('url', 'N/A')}")
						print(f"    Content length: {len(iframe['iframeContent'].get('bodyHTML', ''))}")
		except Exception as e:
		...
		try:
			summary_result = await page.evaluate("() => window.extractIframesBestSummary()")
			...
			if summary_result['summary']:
				...
				for iframe in summary_result['summary']:
					print(f"  - {iframe['hostTag']} (class: {iframe['hostClass']}, id: {iframe['hostId']}, src: {iframe['src']})")
					print(f"    Accessible: {iframe['accessible']}, Has content: {iframe['hasContent']}, Content length: {iframe['contentLength']}")
			else:
		...
		try:
			...
			for result in content_result['contentResults']:
				print(f"  - Iframe {result['index']}: {result['tagName']} (id: {result['id']}, src: {result['src']})")
				print(f"    Accessible: {result['accessible']}")
				if result['error']:
					print(f"    Error: {result['error']}")
				elif result['content']:
					print(f"    Title: {result['content'].get('title', 'N/A')}")
					print(f"    Content length: {len(result['content'].get('bodyHTML', ''))}")
		except Exception as e:
